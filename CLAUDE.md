...
│   └── ChannelManager  # 채널 생성/삭제/조회
├── message/        # 메시지 처리
│   └── MessageDispatcher  # 패킷 타입별 핸들러 라우팅
├── protocol/       # 네트워크 프로토콜
...
`Assets/Plugins/UniversalChat/` 경로에 Unity 패키지로 제공:

- **ChatManager**: MonoBehaviour 싱글톤, 서버 연결 및 이벤트 관리
- **ChatClient**: 저수준 TCP 클라이언트
